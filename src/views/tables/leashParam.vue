<template>
  <div>
        <wizard-step
          :show1=true
          from='groundWireParam'
          next='tensionMachineParam'
        ></wizard-step>
        <a-card class="containerScale" title="选取牵引绳规格" :bordered="false" style="width: 80%;top: 10px;margin: 0 auto;">
            <div>
    ...
    methods: {
        ...
        changesData () {
            ...
            if(this.$refs.importData.files[0].name.match(patt) && this.$refs.importData.files[0].name.match(/\.(?:csv|xls|xlsx)$/i)) {
                ...
                this.$message.error('需要上传包含"牵引绳"命名的csv文件')
            }
        },
        ...
        delItem() {
            if(this.selectedRows.length == 0) {
                ...
                this.$store.commit('renewData', { name:'leash',data:dataSource })
            }
        },
