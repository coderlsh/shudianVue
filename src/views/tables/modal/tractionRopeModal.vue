<template>
  <div class="alarm-modal" ref="exhibitObjModal">
      <a-modal
        ...
        :bodyStyle="{ height: '800px',width: '1610px'}"
        wrapClassName="modal"
        @cancel="handleCancle"
        :getContainer="() => this.$refs.exhibitObjModal"
      >
        <div class="CurveContainer">
            <a-card title="选取牵引绳规格" :bordered="false" style="width: 80%;margin: 0 auto;">
                <div>
                    <a-button @click="selectItem" class="mrb-10" type="primary">选取</a-button>
                    <a-button @click="cancelSelectItem" class="mrb-10" type="primary">取消</a-button>
...
export default {
    ...
    watch: {
      '$store.state.leashParam'() {
        this.dataSource = this.$store.state.leashParam
    ...
    methods: {
      ...
      selectItem() {
        let nowItem = this.selectedRows
        
        this.selectedRowKeys = []
        this.selectedRows = []
        //console.log(this.selectedRows)
        for(let i=0;i<nowItem.length;i++){
          this.$emit('getSelectItem', nowItem[i])
        }
