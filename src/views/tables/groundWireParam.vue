<template>
  <div>
        <wizard-step
          ...
          next='leashParam'
        ></wizard-step>
        <a-card class="containerScale" title="选取地线规格" :bordered="false" style="width: 80%;top: 10px;margin: 0 auto;">
            <div>
  ...
  export default {
    ...
    watch: {
        ...
        '$store.state.groundwireParam'() {
            ...
            this.dataSource = this.$store.state.groundwireParam
    ...
    methods: {
        ...
        changesData () {
            ...
            if(this.$refs.importData.files[0].name.match(patt) && this.$refs.importData.files[0].name.match(/\.(?:csv|xls|xlsx)$/i)) {
                ...
                this.$message.error('需要上传包含"地线"命名的csv文件')
            }
        },
        ...
        delItem() {
            if(this.selectedRows.length == 0) {
                ...
                this.$store.commit('renewData', { name:'groundWire',data:dataSource })
            }
        },
